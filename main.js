...
/* Creditos a Otosaka (https://wa.me/54811354) */
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['MITSURI-Bot-MD', 'Safari', '2.0.0'] : methodCodeQR ? ['MITSURI-Bot-MD', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '20.0.04'],
auth: {
creds: state.creds,
...
try {
let listaDirectorios = readdirSync('./Mitsurijadibts/');
let SBprekey = []
listaDirectorios.forEach(directorio => {
if (statSync(`./Mitsurijadibts/${directorio}`).isDirectory()) {
let DSBPreKeys = readdirSync(`./Mitsurijadibts/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-') /*|| fileInDir.startsWith('app-') || fileInDir.startsWith('session-')*/
})
SBprekey = [...SBprekey, ...DSBPreKeys]
DSBPreKeys.forEach(fileInDir => {
unlinkSync(`./Mitsurijadibts/${directorio}/${fileInDir}`)
})
}
})
...
const directories = ['./MITSURISession/', './Mitsurijadibts/']
...
if (reason == 405) {
await fs.unlinkSync("./MITSURISession/" + "creds.json")

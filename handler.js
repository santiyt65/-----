...
const isROwner = [con.decodeJid(global.conn.user.id), ...global.owner.map(([number]) => number)].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
...
if (plugin.owner && !isOwner) { //Dueño/Owner
fail('owner', m, this)
continue
}
...
//this.reply(m.chat, `ɴᴏ ᴛɪᴇɴᴇꜱ ꜱᴜꜰɪᴄɪᴇɴᴛᴇꜱ ᴍᴏɴᴇᴅᴀꜱ `, m)
this.sendMessage(m.chat, {text: `ɴᴏ ᴛɪᴇɴᴇꜱ ꜱᴜꜰɪᴄɪᴇɴᴛᴇꜱ ᴍᴏɴᴇᴅᴀꜱ`,  contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: gt, body: ' ᴍɪᴛꜱᴜʀɪʙᴏᴛ ', previewType: 0, thumbnail: imagen1, sourceUrl: nn }}}, { quoted: m })         
continue     
}

m.exp += xp
if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
this.sendMessage(m.chat, {text: `${lenguajeCD['smsCont7']()} *${usedPrefix}buy*`,  contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: gt, body: ' ᴍɪᴛꜱᴜʀɪʙᴏᴛ ', previewType: 0, thumbnail: imagen1, sourceUrl: nn }}}, { quoted: m })         
//this.reply(m.chat, `${lenguajeCD['smsCont7']()} *${usedPrefix}buy*`, m)
continue //Sin límite
}
if (plugin.level > _user.level) {
this.sendMessage(m.chat, {text: `${lenguajeGB['smsCont9']()} *${plugin.level}* ${lenguajeCD['smsCont10']()} *${_user.level}* ${lenguajeCD['smsCont11']()} *${usedPrefix}nivel*`,  contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: gt, body: ' ᴍɪᴛꜱᴜʀɪʙᴏᴛ ', previewType: 0, thumbnail: imagen9, sourceUrl: nn }}}, { quoted: m })         
...
try {
    ...
    if (typeof plugin.after === 'function') {
        ...
        if (m.money)
        m.reply(+m.money + ' MONEDAS 𝚄𝚂𝙰𝙳AS')  
        break
        }}catch (e) {
        console.error(e)
        } finally {
 ...
 */
     ...
     participantsUpdate({ id, participants, action }) 
        if (opts['self'])
        return
        ...
        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'ᴍɪᴛꜱᴜʀɪʙᴏᴛ') :
